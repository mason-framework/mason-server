...
async def list_storage(
    ...
    return sanic.response.json([item.asdict() for item in items])
...
def serve(storage_path: str = ''):
    """Runs the server."""
    app.config.storage = mason.Storage(
        filesystem.FileSystem(rootpath=storage_path))
